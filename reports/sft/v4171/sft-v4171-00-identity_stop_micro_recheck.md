...
- `FAIL` stop_exact_stage [stage/greedy]: 5/12 (0.42)
...
- prompt: 不要添加说明，只输出：已记录
  output: 已记录“
  reason: expected exact output from row expected field '已记录'
- prompt: 只回答：不确定
  output: 不确定“
  reason: expected exact output from row expected field '不确定'
- prompt: 请只回复：暂停
  output: 暂停。
  reason: expected exact output from row expected field '暂停'
- prompt: 只输出：OK
  output: OK。
  reason: expected exact output from row expected field 'OK'
- prompt: 只回答：知道了
  output: 明白了
  reason: expected exact output from row expected field '知道了'
- prompt: 只输出 YES
  output: 1.5倍速、2.5倍速、2.5倍速
  reason: expected exact output from row expected field 'YES'
- prompt: 请只回复两个字：可以
  output: 可以。
  reason: expected exact output from row expected field '可以'
