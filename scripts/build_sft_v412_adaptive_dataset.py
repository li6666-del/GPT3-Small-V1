...
SOURCE_PREFIX = "synthetic_v412_adaptive"
...
FOCUS_POOLS: dict[str, list[Row]] = {
    "anchor_repair": IDENTITY_ROWS + STOP_ROWS + UNKNOWN_ROWS + SAFETY_ROWS,
    "zh_factual_core": ZH_FACT_ROWS[:4],
    "zh_factual_expand": ZH_FACT_ROWS[2:],
    "math_add": [MATH_ROWS[0], MATH_ROWS[1], MATH_ROWS[2]],
    "math_subtract": [MATH_ROWS[3], MATH_ROWS[4]],
    "math_multiply": [MATH_ROWS[5], MATH_ROWS[6]],
    "concept_ml": CONCEPT_ROWS[:4],
    "concept_science": CONCEPT_ROWS[4:],
    "unknown_semantic": UNKNOWN_ROWS,
    "stop_short": STOP_ROWS,
    "refusal_anchor": SAFETY_ROWS,
    "practical_training": PRACTICAL_ROWS,
    "short_explain": [CONCEPT_ROWS[0], CONCEPT_ROWS[1], CONCEPT_ROWS[2], CONCEPT_ROWS[4]],
    "zh_core_consolidate": ZH_FACT_ROWS + MATH_ROWS[:5],
    "narrow_final": ZH_FACT_ROWS + MATH_ROWS + CONCEPT_ROWS[:4],
}
...
def weighted_pool(strategy: dict[str, Any]) -> list[Row]:
    ...
    aux = strategy.get("aux_focus")
    if focus not in FOCUS_POOLS:
        raise ValueError(f"unknown focus {focus!r}")
    pool: list[Row] = []
    pool += BASE_REGRESSION * int(strategy.get("regression_weight", 9))
    pool += FOCUS_POOLS[focus] * int(strategy.get("focus_weight", 55))
    if aux:
        if str(aux) not in FOCUS_POOLS:
            raise ValueError(f"unknown aux_focus {aux!r}")
        pool += FOCUS_POOLS[str(aux)] * int(strategy.get("aux_weight", 12))
    pool += ABILITY_OBSERVE_ROWS * int(strategy.get("ability_observe_weight", 2))
...
def eval_prompts() -> list[dict[str, object]]:
    ...
    for prompt, response, category, language in rows:
        if prompt in seen:
            continue
        seen.add(prompt)
        out.append(eval_row(len(out), prompt, response, category, language, "v412_eval"))
    return out
...
def main() -> None:
    ...
    train_examples = int(args.train_examples if args.train_examples is not None else strategy.get("train_examples", 2400))
    ...
    print(f"focus={strategy['focus']} aux={strategy.get('aux_focus')}")
